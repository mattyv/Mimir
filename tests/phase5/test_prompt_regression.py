...
@pytest.mark.phase5
def test_spine_prompt_includes_entity_types(core_vocabulary: object) -> None:
    ...
    missing = [e.iri for e in vocab.entity_types if e.iri not in prompt]
    assert not missing, f"Entity type IRIs missing from spine prompt: {missing}"


@pytest.mark.phase5
def test_spine_prompt_includes_predicates(core_vocabulary: object) -> None:
    ...
    prompt = build_spine_prompt("test content")
    missing = [p.iri for p in vocab.predicates if p.iri not in prompt]
    assert not missing, f"Predicate IRIs missing from spine prompt: {missing}"
...
def test_observations_prompt_includes_all_obs_types() -> None:
    ...
    missing = [t for t in obs_types if t not in prompt]
    assert not missing, f"Observation types missing from observations prompt: {missing}"
